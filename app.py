...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .ds38-card, .limit-card, .info-card {
...
def extraer_coordenadas_google_maps(texto):
    ...
    # Prioriza pares con rango esperable para Chile central:
    # latitud negativa entre -18 y -56; longitud negativa entre -66 y -76.
...
def homologar_por_tabla_prc(comuna, zona_prc, nombre_zona):
    ...
    # IMPORTANTE:
    # No usar un respaldo por código de zona sin comuna.
    # Códigos como H1, H2, ZC, etc. no tienen significado universal
    # entre distintos PRC y pueden provocar que una regla de otra comuna
    # se aplique erróneamente.
    #
    # Si no existe coincidencia por comuna + zona o comuna + nombre,
    # se retorna None para que la homologación continúe con la detección
    # de categorías desde los atributos del IPT correspondiente.
    return None
...
def detectar_comuna_por_punto(lat, lon, indice, gdf_prms_uso_total=None):
    ...
    try:
        ...
        # 1) Intento rápido usando PRMS_USO_Suelo total
        ...
        # 2) Respaldo: buscar en los polígonos comunales disponibles
...
if "gps_precision_m" not in st.session_state:
    st.session_state.gps_precision_m = None
...
st.subheader("Paso 1: Seleccionar comuna")

if "comuna_actual" not in st.session_state:
    st.session_state.comuna_actual = comunas_ordenadas[0]

if st.session_state.comuna_actual not in comunas_ordenadas:
    st.session_state.comuna_actual = comunas_ordenadas[0]

default_index = 0

if st.session_state.comuna_actual in comunas_ordenadas:
    default_index = comunas_ordenadas.index(st.session_state.comuna_actual)
...
if st.session_state.comuna_actual != comuna_clave:
    # Cambio manual de comuna: se limpia el punto anterior
    st.session_state.lat = None
    st.session_state.lon = None
    st.session_state.gps_precision_m = None
...
st.success(f"Comuna activa: {comuna_info['nombre']}")
...
st.subheader("Paso 2: Pegar coordenadas desde Google Maps")

st.info(
    "En el celular: abre Google Maps → mantén presionado el punto → copia las coordenadas → pégalas aquí. "
    "Ejemplo: -33.4175459, -70.5727107. También acepta enlaces largos de Google Maps."
)

coord_google = st.text_input(
    "Coordenadas o enlace de Google Maps",
    placeholder="-33.4175459, -70.5727107"
)

if st.button("Usar coordenadas de Google Maps"):
    lat_google, lon_google = extraer_coordenadas_google_maps(coord_google)

    if lat_google is None or lon_google is None:
        st.error(
            "No pude leer coordenadas válidas. Usa el formato: -33.4175459, -70.5727107 "
            "o pega un enlace largo de Google Maps que contenga las coordenadas."
        )
    else:
        st.session_state.lat = lat_google
        st.session_state.lon = lon_google
        st.session_state.gps_precision_m = None

        comuna_google = detectar_comuna_por_punto(
            lat_google,
            lon_google,
            indice,
            gdf_prms_uso_total
        ...
        if comuna_google and comuna_google in indice:
            st.session_state.comuna_actual = comuna_google
            st.success(
                f"Coordenadas cargadas. Comuna detectada automáticamente: {indice[comuna_google]['nombre']}."
            )
        else:
            st.success("Coordenadas cargadas correctamente. Selecciona la comuna manualmente si el resultado no corresponde.")
...
st.subheader("Paso 2: Buscar dirección")

st.info(
    f"Antes de buscar, verifique que la comuna activa sea **{comuna_info['nombre']}**. "
    "El buscador usará esa comuna para evitar coincidencias en otro sector."
)
...
if st.button("Buscar dirección y homologar"):
    if not direccion.strip():
        st.error("Ingresa una dirección antes de buscar.")
    else:
        ...
        # Se fuerza la búsqueda a la comuna seleccionada para evitar coincidencias en otra comuna.
        consulta = f"{direccion}, {comuna_info['nombre']}, Región Metropolitana, Chile"

        try:
            with st.spinner(f"Buscando dirección en {comuna_info['nombre']}..."):
            ...
            if location:
                lat_dir = float(location.latitude)
                lon_dir = float(location.longitude)

                comuna_dir = detectar_comuna_por_punto(
                    lat_dir,
                    lon_dir,
                    indice,
                    gdf_prms_uso_total
                ...
                if comuna_dir and comuna_dir in indice and comuna_dir != st.session_state.comuna_actual:
                    st.warning(
                        f"La dirección fue ubicada en {indice[comuna_dir]['nombre']}, "
                        f"pero la comuna activa es {comuna_info['nombre']}. "
                        "Cambiaré automáticamente la comuna para evitar una homologación incorrecta."
                    )
                    st.session_state.comuna_actual = comuna_dir

                st.session_state.lat = lat_dir
                st.session_state.lon = lon_dir
                st.session_state.gps_precision_m = None

                st.success(
                    f"Dirección encontrada. Coordenadas: {lat_dir}, {lon_dir}."
                )
                st.rerun()

            else:
                st.error(
                    f"No se encontró la dirección dentro de {comuna_info['nombre']}. "
                    "Para mayor precisión, pegue las coordenadas desde Google Maps."
                )

        except Exception as e:
            st.warning(
                "El servicio de búsqueda de direcciones no respondió correctamente. "
                "Use coordenadas de Google Maps para mayor precisión."
            )
...
st.subheader("Paso 2: Ingresar coordenadas manualmente")
...
if st.button("Homologar coordenadas"):
    ...
    comuna_manual = detectar_comuna_por_punto(
        lat_manual,
        lon_manual,
        indice,
        gdf_prms_uso_total
    ...
    if comuna_manual and comuna_manual in indice:
        st.session_state.comuna_actual = comuna_manual
        st.success(f"Coordenadas cargadas. Comuna detectada automáticamente: {indice[comuna_manual]['nombre']}.")
        st.rerun()
    else:
...
if st.session_state.lat is not None and st.session_state.lon is not None:
    folium.Marker(
        ...
        popup=(
            "Punto seleccionado"
            + (
                f"<br>Precisión aprox.: {round(float(st.session_state.gps_precision_m), 1)} m"
                if st.session_state.get("gps_precision_m") else ""
            )
        ),
